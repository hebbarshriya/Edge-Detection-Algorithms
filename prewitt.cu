...
__global__ void prewittEdgeDetection(unsigned char *input, unsigned char *output, int width, int height)
{
...
int main()
{
    ...
    prewittEdgeDetection<<<blocks, threadsPerBlock>>>(d_input, d_output, width, height);
    cudaDeviceSynchronize(); // Wait for the GPU to finish

    end = clock();
    cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("Prewitt filter on GPU took %f seconds to execute \n", cpu_time_used);

    cudaMemcpy(dst, d_output, width * height, cudaMemcpyDeviceToHost);

    stbi_write_png("output_prewitt.png", width, height, 1, dst, width);
